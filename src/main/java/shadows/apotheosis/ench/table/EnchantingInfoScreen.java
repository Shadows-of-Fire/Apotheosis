...
public class EnchantingInfoScreen extends Screen {
	...
	public EnchantingInfoScreen(ApothEnchantScreen parent) {
		super(new TranslationTextComponent("info.apotheosis.enchinfo_title"));
		this.parent = parent;
		this.imageWidth = 240;
	...
	protected void init() {
		...
		this.slider = this.addButton(new PowerSlider(this.leftPos + 5, this.topPos + 80, 80, 20));
	}
	...
	public void render(MatrixStack pPoseStack, int pMouseX, int pMouseY, float pPartialTick) {
		...
		list.add(new TranslationTextComponent("info.apotheosis.weights").withStyle(TextFormatting.UNDERLINE, TextFormatting.YELLOW));
		...
		for (int i = 0; i < 3; i++) {
			if (this.isHovering(8, 18 + 19 * i, 18, 18, pMouseX, pMouseY)) {
				list.clear();
				list.add(new TranslationTextComponent("info.apotheosis.enchinfo_slot", i + 1).withStyle(TextFormatting.GOLD, TextFormatting.UNDERLINE));
				list.add(new TranslationTextComponent("info.apotheosis.enchinfo_level", this.costs[i]).withStyle(TextFormatting.GREEN));
				list.add(new TranslationTextComponent("info.apotheosis.enchinfo_minpow", this.powers[i][0]).withStyle(TextFormatting.DARK_RED));
				list.add(new TranslationTextComponent("info.apotheosis.enchinfo_maxpow", this.powers[i][1]).withStyle(TextFormatting.BLUE));
		...
		if (hover != null) {
			list.clear();
			list.add(new TranslationTextComponent(hover.data.enchantment.getDescriptionId()).withStyle(TextFormatting.GREEN, TextFormatting.UNDERLINE));
			list.add(new TranslationTextComponent("info.apotheosis.enchinfo_level", new TranslationTextComponent("enchantment.level." + hover.data.level)).withStyle(TextFormatting.DARK_AQUA));
			ITextComponent rarity = new TranslationTextComponent("rarity.enchantment." + hover.data.enchantment.getRarity().name().toLowerCase(Locale.ROOT)).withStyle(colors[hover.data.enchantment.getRarity().ordinal()]);
			list.add(new TranslationTextComponent("info.apotheosis.enchinfo_rarity", rarity).withStyle(TextFormatting.DARK_AQUA));
			list.add(new TranslationTextComponent("info.apotheosis.enchinfo_chance", String.format("%.2f", 100F * hover.weight / WeightedRandom.getTotalWeight(this.enchantments)) + "%").withStyle(TextFormatting.DARK_AQUA));
			if (I18n.exists(hover.data.enchantment.getDescriptionId() + ".desc")) {
				list.add(new TranslationTextComponent(hover.data.enchantment.getDescriptionId() + ".desc").withStyle(TextFormatting.DARK_AQUA));
			}
			...
			if (!excls.isEmpty()) {
				...
				list.add(new TranslationTextComponent("Exclusive With: %s", sb.toString()).withStyle(TextFormatting.RED));
			}
			this.renderComponentTooltip(pPoseStack, list, pMouseX, pMouseY);
		}
	...
	public class PowerSlider extends AbstractSlider {
		...
		protected void updateMessage() {
			this.setMessage(new TranslationTextComponent("info.apotheosis.slider_power", EnchantingInfoScreen.this.currentPower));
		}
		...
		public double normalizeValue(double value) {
			return MathHelper.clamp((this.snapToStepClamp(value) - this.min()) / (this.max() - this.min()), 0.0D, 1.0D);
		}
		...
		public int denormalizeValue(double value) {
			return (int) this.snapToStepClamp(MathHelper.lerp(MathHelper.clamp(value, 0.0D, 1.0D), this.min(), this.max()));
		}
		...
		private double snapToStepClamp(double valueIn) {
			...
			return MathHelper.clamp(valueIn, this.min(), this.max());
		}
