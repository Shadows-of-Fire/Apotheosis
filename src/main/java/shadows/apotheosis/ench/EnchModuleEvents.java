...
public class EnchModuleEvents {
	...
	public void anvilEvent(AnvilUpdateEvent e) {
		...
		/*
		if (e.getLeft().getItem() == Apoth.Blocks.HELLSHELF.asItem() || e.getLeft().getItem() == ApotheosisObjects.SEASHELF.asItem()) {
			if (e.getLeft().getItem() != e.getRight().getItem() || e.getLeft().getCount() != 1) return;
			Enchantment ench = e.getLeft().getItem() == Apoth.Blocks.HELLSHELF.asItem() ? ApotheosisObjects.HELL_INFUSION : ApotheosisObjects.SEA_INFUSION;
