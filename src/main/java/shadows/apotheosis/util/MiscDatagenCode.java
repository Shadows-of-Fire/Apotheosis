...
public class MiscDatagenCode {
	...
	public static void genAffixLootItems() {
		Gson gson = new GsonBuilder().registerTypeAdapter(ItemStack.class, ItemAdapter.INSTANCE).registerTypeAdapter(CompoundTag.class, NBTAdapter.INSTANCE).registerTypeAdapter(ResourceLocation.class, new ResourceLocation.Serializer()).setPrettyPrinting().create();
		...
		for (Item i : ForgeRegistries.ITEMS) {
			if (i instanceof TieredItem t) {
				...
				if (tier != Tiers.WOOD && tier.ordinal() <= Tiers.IRON.ordinal()) {
					AffixLootEntry entry = new AffixLootEntry(100 - 15 * (tier.ordinal() - 1), 1 + 3 * (tier.ordinal() - 1), new ItemStack(i), LootCategory.forItem(new ItemStack(i)), overworld);
				...
				if (tier.ordinal() >= Tiers.IRON.ordinal() && tier.ordinal() <= Tiers.DIAMOND.ordinal() || tier == Tiers.GOLD) {
					...
					AffixLootEntry entry = new AffixLootEntry(weight, quality, new ItemStack(i), LootCategory.forItem(new ItemStack(i)), nether);
				...
				if (tier == Tiers.DIAMOND || tier == Tiers.NETHERITE) {
					...
					AffixLootEntry entry = new AffixLootEntry(weight, quality, new ItemStack(i), LootCategory.forItem(new ItemStack(i)), end);
				...
				if (mat.ordinal() <= ArmorMaterials.IRON.ordinal()) {
					AffixLootEntry entry = new AffixLootEntry(100 - 15 * (mat.ordinal()), 1 + 2 * (mat.ordinal()), new ItemStack(i), LootCategory.ARMOR, overworld);
					writerFunc.accept("overworld", entry);
				}

				if (mat.ordinal() >= ArmorMaterials.IRON.ordinal() && mat.ordinal() <= ArmorMaterials.DIAMOND.ordinal()) {
					AffixLootEntry entry = new AffixLootEntry(100 - 15 * (mat.ordinal() - 2), 1 + 2 * (mat.ordinal() - 2), new ItemStack(i), LootCategory.ARMOR, nether);
					writerFunc.accept("the_nether", entry);
				}
				...
				if (mat == ArmorMaterials.DIAMOND || mat == ArmorMaterials.NETHERITE) {
					...
					AffixLootEntry entry = new AffixLootEntry(weight, quality, new ItemStack(i), LootCategory.ARMOR, end);
					writerFunc.accept("the_end", entry);
				}
			}
		}
