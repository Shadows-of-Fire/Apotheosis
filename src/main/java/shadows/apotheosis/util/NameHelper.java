	...
	*/
	...
	public static String setEntityName(Random random, Mob entity) {
		...
		if (names.length > 0 && nameParts.length > 0) {
			root = random.nextFloat() < 0.8F ? NameHelper.names[random.nextInt(NameHelper.names.length)] : NameHelper.nameFromParts(random);
		...
		String name = root;
		if (random.nextFloat() < 0.3F && prefixes.length > 0) {
			name = NameHelper.prefixes[random.nextInt(NameHelper.prefixes.length)] + " " + name;
		}
