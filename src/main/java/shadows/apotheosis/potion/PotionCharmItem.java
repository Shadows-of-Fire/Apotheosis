	...
	public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean isSelected) {
		...
		if (stack.getOrCreateTag().getBoolean("charm_enabled") && entity instanceof ServerPlayerEntity) {
			...
			if (active == null || active.getDuration() < (active.getPotion() == Effects.NIGHT_VISION ? 210 : 5)) {
				((ServerPlayerEntity) entity).addPotionEffect(new EffectInstance(contained.getPotion(), contained.getDuration() / 24 + (contained.getPotion() == Effects.NIGHT_VISION ? 210 : 5), 0, false, false));
