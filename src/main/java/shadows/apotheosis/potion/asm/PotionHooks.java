...
public class PotionHooks {
	...
	public static boolean isInfinite(ItemStack stack, ItemStack bow, PlayerEntity player) {
		...
		return EnchantmentHelper.getEnchantmentLevel(ApotheosisObjects.TRUE_INFINITY, bow) > 0 && stack.getItem() instanceof ArrowItem;
	}
	...
	public static float applyPotionDamageCalculations(LivingEntity entity, DamageSource source, float damage) {
		if (source.isDamageAbsolute()) {
			...
			if (entity.isPotionActive(Effects.RESISTANCE) && source != DamageSource.OUT_OF_WORLD) {
				int level = entity.getActivePotionEffect(Effects.RESISTANCE).getAmplifier() + 1;
				mult -= 0.2 * level;
			}
