...
public class EnlightenedAffix extends Affix {
	...
	protected static final Float2IntFunction COST_FUNC = AffixHelper.step(9, 8, -1);
	...
	public void addInformation(ItemStack stack, LootRarity rarity, float level, Consumer<Component> list) {
		list.accept(new TranslatableComponent("affix." + this.getRegistryName() + ".desc", COST_FUNC.get(level)).withStyle(ChatFormatting.YELLOW));
	}
	...
	public InteractionResult onItemUse(ItemStack stack, LootRarity rarity, float level, UseOnContext ctx) {
		...
		if (AdventureConfig.torchItem.get().useOn(ctx).consumesAction()) {
			...
			player.getItemInHand(ctx.getHand()).hurtAndBreak(COST_FUNC.get(level), player, p -> p.broadcastBreakEvent(ctx.getHand()));
