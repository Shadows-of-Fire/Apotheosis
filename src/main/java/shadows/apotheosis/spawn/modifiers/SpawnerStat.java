...
public interface SpawnerStat<T> {
	...
	default TranslationTextComponent name() {
		return new TranslationTextComponent("stat.apotheosis." + getId());
	}

	default TranslationTextComponent desc() {
		return new TranslationTextComponent("stat.apotheosis." + getId() + ".desc");
	}
}
