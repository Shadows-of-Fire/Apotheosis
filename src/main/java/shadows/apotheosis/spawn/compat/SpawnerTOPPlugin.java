...
public class SpawnerTOPPlugin implements TOPCompat.Provider {
	...
	public void addProbeInfo(ProbeMode mode, IProbeInfo info, PlayerEntity player, World level, BlockState state, IProbeHitData hitData) {
		...
		if (te instanceof ApothSpawnerTile) {
			...
			if (spw.ignoresPlayers) info.mcText(SpawnerStats.IGNORE_PLAYERS.name().withStyle(TextFormatting.DARK_GREEN));
			if (spw.ignoresConditions) info.mcText(SpawnerStats.IGNORE_CONDITIONS.name().withStyle(TextFormatting.DARK_GREEN));
