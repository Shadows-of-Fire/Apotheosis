...
// Disabled: WandererTradesRegistry and WandererTrade interface removed from Placebo 10.0.0.
// Restore when the wanderer trade system is available again.
...
//         }
//         else {
//             List<Weighted<AffixLootEntry>> resolved = this.entries.stream().map(this::unwrap).filter(Objects::nonNull).map(e -> e.<AffixLootEntry>wrap(ctx.tier(), ctx.luck())).toList();
//             AffixLootEntry entry = WeightedRandom.getRandomItem(rand, resolved, Weighted::weight).get().value();
